...
public class Grid {
	...
	public Grid(int width, int height, Bounds bounds) {
		...
		bandwidthX = bounds.size().x / (width - 1);
		bandwidthY = bounds.size().y / (height- 1);
	...
	public float[] getData(Bounds bounds, int[] gridSize) {
		Vec2 p0 = points[0];
		int i1 = (int) (Math.abs(bounds.getP1().x - p0.x) / bandwidthX);
		int j1 = (int) (Math.abs(bounds.getP1().y - p0.y) / bandwidthY);
		int i2 = (int) (Math.abs(bounds.getP2().x - p0.x) / bandwidthX);
		int j2 = (int) (Math.abs(bounds.getP2().y - p0.y) / bandwidthY);
		int height = j1 - j2;
		int width = i2-i1;
	...
	public void printData() {
		...
		for (int i = 0; i < width; i++) {
			for (int j = 0; j < height; j++) {
				String d = String.format("%.4f", data[i*height + j]).replace(',', '.');
	...
	public void evaluate(QuadTree qTree, Kernel k) {
		...
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				data[i*height + j] = evaluatePointQuadTree(qTree, bandwidthX, k, points[i*width + j]);
			}
		}
	}
	...
	private void valuate(List<Vec2> values, Kernel k, int evalDeval) {
		...
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				data[i*width+ j] += evalDeval * evaluatePointGrid(pointListCellGrid, i, j, k);
			}
		}
	}
	...
	private float evaluatePointGrid(List<Vec2> pointListCellGrid[][], int i, int j, Kernel k) {
		...
		for (float distance : distances) {
			valueAtP += k.eval(distance / bandwidthX);
		}
	...
	private void createPointListCellGrid(List<Vec2> values, List<Vec2>[][] pointListCellGrid) {
		...
		for (Vec2 value : values) {
			int j = (int) (Math.abs(value.x - p0.x) / bandwidthX);
			int i = (int) (Math.abs(value.y - p0.y) / bandwidthY);
			if ( i == height-1) --i;
			if ( j == width-1) --j;
