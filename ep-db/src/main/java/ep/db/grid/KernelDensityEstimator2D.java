...
public class KernelDensityEstimator2D {
	...
	public void compute(DoubleMatrix2D data, double[] h, double[] bounds){
	...
	public static void main(String[] args) {
		...
		File file = new File("/Users/jose/Documents/freelancer/petricaep/points50.csv");
...
//		KernelFunction kernel = new QuarticKernelFunction();
		double[] h = new double[]{2.0/49.0,2.0/49.0};
		KernelDensityEstimator2D kde = new KernelDensityEstimator2D(kernel, 50);
		kde.compute(new DenseDoubleMatrix2D(data), h, new double[]{-1,-1,1,1});
		
		double[][] densities = kde.getKDE();
		for(i = 0; i < densities.length; i++){
			for( int j = 0; j < densities[i].length; j++){
				System.out.print(densities[i][j] + ",");
			}
			System.out.println();
		}
