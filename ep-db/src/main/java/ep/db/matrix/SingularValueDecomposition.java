...
//					A[k][k] += 1.0;
			...
			for (int j = k+1; j < n; j++) {
				if ((k < nct) && (s[k] != 0.0))  {
					...
					for (int i = k; i < m; i++) {
						Vector vi = matrix.getRow(i);
						t += vi.getValue(k) * vi.getValue(j);
					}
					t = -t/row.getValue(k); //A[k][k];
					for (int i = k; i < m; i++) {
						Vector vi = matrix.getRow(i);
...
//						A[i][j] += t*A[i][k];
			...
			if (wantu && (k < nct)) {
				...
				for (int i = k; i < m; i++) {
					U[i][k] = matrix.getRow(i).getValue(k); //A[i][k];
				}
			}
			if (k < nrt) {
				...
				if ((k+1 < m) && (e[k] != 0.0)) {
					...
					for (int j = k+1; j < n; j++) {
						for (int i = k+1; i < m; i++) {
							work[i] += e[j] * matrix.getRow(i).getValue(j);
						}
					}
					for (int j = k+1; j < n; j++) {
						float t = -e[j]/e[k+1];
...
//							A[i][j] += t*work[i];
		...
		if (nct < n) {
			s[nct] = matrix.getRow(nct).getValue(nct);
		}
		if (m < p) {
			s[p-1] = 0.0f;
		}
		if (nrt+1 < p) {
			e[nrt] = matrix.getRow(nrt).getValue(p-1); //A[nrt][p-1];
		}
		e[p-1] = 0.0f;
